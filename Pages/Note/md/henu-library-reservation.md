...
| 人不在馆内时签到 | `code=358 请通过门禁入馆后操作` | **门禁状态门**：全程无经纬度参数，不是内网限制；人在馆内同一调用 code 0。尝试零副作用，进馆刷卡即自动签到 |
...
> ✅ 保留意见已收敛（出馆实测）：人在馆外时调签到回 `358`，人在馆内同一调用成功——门限是**门禁状态**而非网络位置。含义：自动签到只在"人已在馆内但还没签"时生效；人没到时尝试零副作用，进馆刷卡即自动签到。系统侧用独立 `waitgate` 状态承载，不误报失败。
