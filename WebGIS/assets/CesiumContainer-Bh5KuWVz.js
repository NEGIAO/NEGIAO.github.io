...
void main() {
  ...
  out_FragColor = vec4(color, alpha);
    ...
    `,document.head.appendChild(C)}}function J(){b=new e.ScreenSpaceEventHandler(n.scene.canvas),b.setInputAction(s=>{const c=n.camera.getPickRay(s.endPosition);if(!c)return;const p=n.scene.globe.pick(c,n.scene);if(p){const D=e.Cartographic.fromCartesian(p),C=e.Math.toDegrees(D.longitude).toFixed(6),v=e.Math.toDegrees(D.latitude).toFixed(6),U=D.height.toFixed(2);T.value=`经度: ${C}, 纬度: ${v}, 海拔: ${U}米`}},e.ScreenSpaceEventType.MOUSE_MOVE)}function Q(){const s=n.imageryLayers,c=new e.UrlTemplateImageryProvider({url:`${ye}DataServer?T=img_w&x={x}&y={y}&l={z}&tk=${r}`,subdomains:o,tilingScheme:new e.WebMercatorTilingScheme,maximumLevel:18});s.addImageryProvider(c);const p=new e.UrlTemplateImageryProvider({url:`${ye}DataServer?T=ibo_w&x={x}&y={y}&l={z}&tk=${r}`,subdomains:o,tilingScheme:new e.WebMercatorTilingScheme,maximumLevel:10});s.addImageryProvider(p)}function A(){var c;const s=o.map(p=>`https://t${p}.tianditu.gov.cn/mapservice/swdx?T=elv_c&tk=${r}`);if(!((c=window.Cesium)!=null&&c.GeoTerrainProvider))return f.error("GeoTerrainProvider 不存在，插件未正确加载。"),!1;try{return n.terrainProvider=new window.Cesium.GeoTerrainProvider({urls:s}),n.scene.globe.depthTestAgainstTerrain=!0,!0}catch(p){return n.terrainProvider=new e.EllipsoidTerrainProvider,f.warning("官方地形服务加载失败，已降级为椭球地形。",{closable:!0}),f.error("官方地形初始化失败",p),!1}}function z(){const D=[0,2e3,5e3,1e4,15e3],C=[3e4,3e4,25e3,25e3,2e4],v=[30,30,25,25,20],U=v.reduce((H,B)=>H+B*B,0),se=new Array(U),le=new Array(U),ce=new Array(U);let de=0;for(let H=0;H<5;H++){const B=v[H],ue=v[H],he=C[H];for(let $=0;$<ue;$++)for(let q=0;q<B;q++){const ne=de+$*B+q,Y=(q-B/2)*(he/111320),te=($-ue/2)*(he/111320/Math.cos(e.Math.toRadians(35))),t=Math.atan2(te,Y)+Math.PI/2+.2*Math.sin(q*.5)*Math.cos($*.5);le[ne]=e.Math.toDegrees(t)%360;const a=Math.sqrt(Y*Y+te*te),_=Math.max(0,1-a/15);se[ne]=(5+H*2)*_+2*Math.random(),ce[ne]=.5*Math.sin(q*.3)*Math.cos($*.3)}de+=B*ue}return{longitude:104,latitude:35,altitude:D,sizeMesh:C,count:v,hspeed:se,hdir:le,vspeed:ce}}function ie(){if(!n||!e){f.error("Cesium 尚未初始化");return}L();const s=z();x.value=new Ke(n,{maxWindSpeed:20,cesium:e,speedFactor:h.value,arrowLength:V.value,trailLength:S.value,alphaFactor:P.value}),x.value.loadData(s),n.scene.primitives.add(x.value),x.value.flyTo(),f.success("风场加载成功，可通过下方滑块调节样式")}function O(){x.value&&(x.value.speedFactor=h.value,x.value.arrowLength=V.value,x.value.trailLength=S.value,x.value.alphaFactor=P.value)}function R(s,c){return new Promise((p,D)=>{const C=document.getElementById(c);if(C){if(C.getAttribute("data-loaded")==="true"){p();return}C.addEventListener("load",()=>p(),{once:!0}),C.addEventListener("error",()=>D(new Error(`脚本加载失败: ${s}`)),{once:!0});return}const v=document.createElement("script");v.id=c,v.src=s,v.async=!0,v.onload=()=>{v.setAttribute("data-loaded","true"),p()},v.onerror=()=>D(new Error(`脚本加载失败: ${s}`)),document.head.appendChild(v)})}function y(s,c){return new Promise((p,D)=>{if(document.getElementById(c)){p();return}const v=document.createElement("link");v.id=c,v.rel="stylesheet",v.href=s,v.onload=()=>p(),v.onerror=()=>D(new Error(`样式加载失败: ${s}`)),document.head.appendChild(v)})}function I(s){if(!n)return;const c=typeof s=="number"?s:2;n.camera.flyTo({destination:e.Cartesian3.fromDegrees(104.1954,35.8617,15e6),orientation:{heading:0,pitch:-e.Math.PI_OVER_TWO,roll:0},duration:c})}function ee(){n&&n.camera.flyTo({destination:e.Cartesian3.fromDegrees(86.925,27.9881,9e3),orientation:{heading:e.Math.toRadians(0),pitch:e.Math.toRadians(-25),roll:0},duration:3})}async function re(){if(n)try{const s=await e.Cesium3DTileset.fromUrl("https://raw.githubusercontent.com/CesiumGS/3d-tiles-samples/master/1.0/TilesetWithDiscreteLOD/tileset.json");n.scene.primitives.add(s),n.flyTo(s,{duration:3,offset:new e.HeadingPitchRange(e.Math.toRadians(0),e.Math.toRadians(-25),s.boundingSphere.radius*2.5)})}catch(s){f.error(`加载模型失败: ${s}`),f.error("加载3D模型失败，可能是网络原因无法访问 GitHub 资源。",{closable:!0,duration:6500})}}return(s,c)=>(_e(),xe(Ee,null,[c[6]||(c[6]=u("div",{id:"cesiumContainer",class:"cesium-container"},null,-1)),g.value?(_e(),we(Me(Ue),{key:0,"get-viewer":X,"get-cesium":k})):ge("",!0),u("div",Qe,[u("div",Ye,oe(T.value),1),c[5]||(c[5]=u("div",{class:"divider"},null,-1)),u("button",{class:"home-btn",onClick:I,title:"回到初始位置"},[...c[4]||(c[4]=[u("svg",{viewBox:"0 0 24 24",width:"18",height:"18",fill:"currentColor"},[u("path",{d:"M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"})],-1)])])]),u("div",{class:"cesium-controls"},[u("button",{onClick:ee,class:"fly-btn"},"🏔️ 飞越珠穆朗玛峰"),u("button",{onClick:re,class:"fly-btn"},"🏢 加载3D模型"),u("button",{onClick:ie,class:"fly-btn"},"🌬️ 加载模拟风场")]),x.value?(_e(),xe("div",Ze,[u("div",Je,[u("label",null,"速度因子: "+oe(h.value.toFixed(1)),1),j(u("input",{type:"range",min:"0.1",max:"5",step:"0.1","onUpdate:modelValue":c[0]||(c[0]=p=>h.value=p),onInput:O},null,544),[[pe,h.value,void 0,{number:!0}]])]),u("div",et,[u("label",null,"箭头长度: "+oe(V.value/1e3)+"km",1),j(u("input",{type:"range",min:"5000",max:"50000",step:"1000","onUpdate:modelValue":c[1]||(c[1]=p=>V.value=p),onInput:O},null,544),[[pe,V.value,void 0,{number:!0}]])]),u("div",tt,[u("label",null,"尾迹长度: "+oe(S.value/1e3)+"km",1),j(u("input",{type:"range",min:"5000",max:"80000",step:"1000","onUpdate:modelValue":c[2]||(c[2]=p=>S.value=p),onInput:O},null,544),[[pe,S.value,void 0,{number:!0}]])]),u("div",it,[u("label",null,"透明度: "+oe(P.value.toFixed(2)),1),j(u("input",{type:"range",min:"0.1",max:"1",step:"0.05","onUpdate:modelValue":c[3]||(c[3]=p=>P.value=p),onInput:O},null,544),[[pe,P.value,void 0,{number:!0}]])])])):ge("",!0)],64))}},_t=Se(at,[["__scopeId","data-v-e6d6448b"]]);export{_t as default};
